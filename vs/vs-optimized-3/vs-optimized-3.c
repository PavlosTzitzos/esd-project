...
int main()
{
    ...
    while (1)
    {
        ...
        for (i = 1;i < NN + 1;i++)
        {
            ...
            {
                b = (double)(
                    *(*(pff + i - 1) + j + 1) - *(*(pff + i - 1) + j - 1) +
                    *(*(pff + i) + j + 1) + *(*(pff + i) + j + 1) - *(*(pff + i) + j - 1) - *(*(pff + i) + j - 1) +
                    *(*(pff + i + 1) + j + 1) - *(*(pff + i + 1) + j - 1));

                c = (double)(
                    *(*(pff + i + 1) + j + 1) + *(*(pff + i + 1) + j - 1) + *(*(pff + i + 1) + j) + *(*(pff + i + 1) + j) -
                    *(*(pff + i - 1) + j + 1) - *(*(pff + i - 1) + j - 1) - *(*(pff + i - 1) + j) - *(*(pff + i - 1) + j));
        ...
        for (i = 0;i < NN + 2;i++)
        {
            ...
            {
                // Loop Unrolling :
                *(*(pff + i) + j) = (int)(slope * (*(*(pff + i) + j) - min) + 0.5);
                *(*(pff + i) + j + 1) = (int)(slope * (*(*(pff + i) + j + 1) - min) + 0.5);
                *(*(pff + i) + j + 2) = (int)(slope * (*(*(pff + i) + j + 2) - min) + 0.5);
                *(*(pff + i) + j + 3) = (int)(slope * (*(*(pff + i) + j + 3) - min) + 0.5);
            }
        }
        ...
        for (i = 0; i < NN; i++)
        {
            for (j = 0; j < MM; j++)
            {
                ...
                if (f >= 67.5 && f <= 90) {
                    ...
                    *(*(pfa + i) + j) = 0;  // Red component
                    *(*(pfb + i) + j) = 0;  // Green component
                    *(*(pfc + i) + j) = g;  // Blue component
                }
                else if (f < 67.5 && f > 22.5) {
                    // Diagonal edge (green)
                    *(*(pfa + i) + j) = 0;  // Red component
                    *(*(pfb + i) + j) = g;  // Green component
                    *(*(pfc + i) + j) = 0;  // Blue component
                }
                else if (f < 22.5 && f > 0) {
                    // Horizontal edge (red)
                    *(*(pfa + i) + j) = g;  // Red component
                    *(*(pfb + i) + j) = 0;  // Green component
                    *(*(pfc + i) + j) = 0;  // Blue component
                }
                else
